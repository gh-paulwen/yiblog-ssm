...
<body>
	...
	<div class="container">
		<div class="row">
			...
			<div class="well">
			  <form:form action="${pageContext.request.contextPath }/category/submitSave" method="POST" modelAttribute="category">
                 <div class="form-group">
                    <h4>添加分类 :</h4>
                    <form:input path="name" class="form-control" placeholder="分类名"/>
                 </div>
                 <input type="submit" class="btn btn-primary" value="提交">
              </form:form>
			</div>
			<div class="well">
			  <form:form action="${pageContext.request.contextPath }/subCategory/submitSave" method="POST" modelAttribute="subCategory">
                 <div class="form-group">
                    <h4>添加子分类 :</h4>
                    <form:select path="category.id" items="${listCategory }" itemLabel="name" itemValue="id"></form:select>
                    <h4></h4>
                    <form:input path="name" class="form-control" placeholder="子分类名"/>
                 </div>
                 <input type="submit" class="btn btn-primary" value="提交">
              </form:form>
			</div>
			...
			<div class="well">
			  <form:form action="${pageContext.request.contextPath }/subCategory/submitUpdate" method="POST" modelAttribute="subCategory">
                 <div class="form-group">
                    <h4>添加子分类的图片路径 :</h4>
                    <form:select path="id" items="${listSubCategory }" itemLabel="name" itemValue="id"></form:select>
                    <h4></h4>
                    <form:input path="logopath" class="form-control" placeholder="图片路径"/>
                 </div>
                 <input type="submit" class="btn btn-primary" value="提交">
              </form:form>
			</div>
